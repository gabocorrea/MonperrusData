...
fdout = open("../out/probable_directives_(comments_with_keywords).csv","w")
fdout2 = open("../out/dataset_probably_with_non_directives.csv","w")
...
fdout2.write("id_phrase,type,path,text\n")
with open("../out/dataset_withID_with_phrases.csv") as fdDataset:
	...
	for line in fdDataset:
		...
		if i>1:
			...
			if int(line_splitted[0].split('-')[0]) not in list_of_directives_id_INT:###########TODO#################333
